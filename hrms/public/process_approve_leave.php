...
// Update leave status
$query = $conn->prepare("UPDATE leave_requests SET status = 'approved', admin_remarks = ? WHERE id = ?");
...
if ($query->execute()) {
    ...
    $message = "Your " . ucfirst($leave['leave_type']) . " leave request from " . date('M j', strtotime($leave['start_date'])) . " to " . date('M j', strtotime($leave['end_date'])) . " has been approved!";
    $type = "success";
